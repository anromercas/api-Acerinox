...
function saveSdr(req, res){
    ...
    sdr.is_it_concrete = params.is_it_concrete;
    sdr.created_by = params.created_by;
    sdr.state = params.state;
    sdr.cost = params.cost;
    sdr.image = params.image;
    sdr.date = params.date;
    sdr.dapertment = params.department;

    sdr.save(function(err,sdrStored){
        if(err) res.status(500).send({message: `Error al guardar SDR ${err}`});
        if(!sdrStored) res.status(404).send({message: 'El sdr no ha sido guardado'});
